<?php
namespace PhalApi\SMS;

use PhalApi\Exception\BadRequestException;
use PhalApi\SMS\SDK\REST as REST;
...
class Lite
{
    ...
    public function __construct($cofAddress, $debug = false) {
        ...
        if ($this->getIndex($config, 'accountSid')) {
            ...
            throw new BadRequestException('accountSid There is no', 1);
        }
        //主帐号Token是否配置
        ...
        //初始化SDK
        $this->SMS_SDK_REST = new REST($this->serverIP, $this->serverPort, $this->softVersion);
        //设置主帐号
    ...
    function voiceVerify($verifyCode, $playTimes, $to, $displayNum, $respUrl, $lang, $userData) {
        ...
        if ($result == NULL) {
            throw new BadRequestException('Error SMS_SDK_REST', 1);
        }
        //如果是Debug模式则打印返回信息
        if ($this->debug) {
            $this->showarr($result);
